...
window.addEventListener(
    ...
    () => {
        if (window.scrollY > 30) {
            ...
            header.style.background = 'rgba(10, 10, 15, 0.68)';
        }
    },
    {
        passive: true
...
projects.forEach(project => {
    project.addEventListener('mousemove', event => {
        const rectangle = project.getBoundingClientRect();
        const x = event.clientX - rectangle.left;
        const percentage = (x / rectangle.width) * 100;

        project.style.background = `
            linear-gradient(
                90deg,
                rgba(113, 87, 255, ${0.03 + percentage / 2500}),
                transparent
            )
        `;
    });
